...
test('BUG5 Storage capacity — "6Gb/s" (SATA speed) is not parsed as 6GB capacity', () => {
  const specs = (t) => extractSpecs(t, 'Storage');
  assert.equal(specs('Bestoss M.2 SATA III 6Gb/s NGFF 128GB SSD 3D NAND Solid State Drive').cap, 128);
  assert.equal(specs('Lexar NQ100 2.5" SATA III 6Gb/s Internal SSD 256GB').cap, 256);
  assert.equal(specs('Fikwot FN203 M.2 SATA SSD 3D NAND TLC SATA III 6Gb/s M.2 2280 1TB').cap, 1000);
  // capacity-before-speed still fine; NVMe (MB/s) unaffected
  assert.equal(specs('Crucial P310 2TB PCIe Gen4 NVMe M.2 2280 SSD, Up to 7100MB/s').cap, 2000);
  assert.equal(specs('WD Blue 4TB SATA 6Gb/s 3.5" HDD').cap, 4000);
});
