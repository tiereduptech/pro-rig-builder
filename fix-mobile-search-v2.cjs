// Patch to add MobileSearchPage component + responsive router
// Uses plain string (not template literal) to avoid $ escape issues

const fs = require('fs');
let s = fs.readFileSync('src/App.jsx', 'utf8');
s = s.replace(/\r\n/g, '\n');

// Build the JSX as a plain (non-template) string via array join
// This avoids any `${...}` template-literal interpretation
const LINES = [
'',
'// === MOBILE SEARCH PAGE ===',
...
'                <span style={{fontFamily:"var(--ff)",fontSize:18,fontWeight:800,color:"var(--mint)"}}>DOLLAR{fmtPrice($(p))}</span>',
...
'                    <span style={{fontFamily:"var(--ff)",fontSize:16,fontWeight:800,color:ri===0?"var(--mint)":"var(--txt)"}}>DOLLAR{fmtPrice(r.price)}</span>',
...
'          <input type="number" value={minPr||""} onChange={e=>setMinPr(+e.target.value||0)} placeholder="Min DOLLAR" style={{flex:1,background:"var(--bg4)",border:"1px solid var(--bdr)",borderRadius:6,padding:"8px 10px",fontSize:13,color:"var(--txt)",fontFamily:"var(--mono)",outline:"none",boxSizing:"border-box"}}/>',
'          <input type="number" value={maxPr>=5000?"":maxPr} onChange={e=>setMaxPr(+e.target.value||5000)} placeholder="Max DOLLAR" style={{flex:1,background:"var(--bg4)",border:"1px solid var(--bdr)",borderRadius:6,padding:"8px 10px",fontSize:13,color:"var(--txt)",fontFamily:"var(--mono)",outline:"none",boxSizing:"border-box"}}/>',
'        </div>',
...
'          <span style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--mute)"}}>DOLLAR{minPr}</span>',
'          <span style={{fontFamily:"var(--mono)",fontSize:11,color:"var(--mint)",fontWeight:600}}>to DOLLAR{maxPr>=5000?"\u221E":"DOLLAR"+maxPr}</span>',
'        </div>',
'',
...
// Join lines, then replace DOLLAR placeholder with actual $ sign
let mobileComponent = LINES.join('\n');
mobileComponent = mobileComponent.split('DOLLAR').join('$');
...
// Also check if MobileSearchPage already present (from previous bad commit)
if (s.includes('function MobileSearchPage(')) {
  console.log('WARN: MobileSearchPage already present - stripping old copy first');
  const startOld = s.indexOf('// === MOBILE SEARCH PAGE ===');
  const endOld = s.indexOf('function SearchPage({activeCat,th}){', startOld);
  if (startOld > 0 && endOld > startOld) {
    s = s.substring(0, startOld) + s.substring(endOld);
  }
}

const idx2 = s.indexOf(insertBefore);
s = s.substring(0, idx2) + mobileComponent + s.substring(idx2);
console.log('PART B OK: MobileSearchPage inserted');
...
if (s.includes(newRoute)) {
  console.log('PART C: route already updated');
} else if (!s.includes(oldRoute)) {
  console.log('ROUTE ANCHOR MISS'); process.exit(1);
} else {
  s = s.replace(oldRoute, newRoute);
  console.log('PART C OK: route updated');
}
