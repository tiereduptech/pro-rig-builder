...
const CADENCE = {
  amazon: {
    confirmedBy: { workflow: 'verify-catalog.yml', cron: '0 10 * * 1' },
    why:
      'verify-catalog.yml runs verify-catalog-asins.js in four tiers — tier 1 every 2 days, ' +
      'tier 2 every 3, tiers 3 and 4 weekly. The weekly tier is the slowest confirmation any ' +
      'Amazon row can expect, so it sets the budget for all of them. This is the one retailer ' +
      'demonstrably working: 2,810 of 3,758 rows confirmed, price changes on 62 separate days.',
  },
  ...
  bestbuy: {
    unscheduled:
      'NO REFRESHER EXISTS — this is not a schedule that lapsed. No script in the repo writes ' +
      'a Best Buy price confirmation: the five writers of priceConfirmedAt (verify-catalog-asins.js, ' +
      'drift-gate.js, apply-amazon-cases.mjs, weekend-amazon-ingest.mjs, case-ingest.mjs) all cover ' +
      'Amazon, Newegg or cases. Prices are captured once at merge time by bestbuy-merge.js and never ' +
      'touched again. The 9 stamped rows of 1,523 are all Cases, a side effect of a one-off ' +
      'case-ingest.mjs run on 2026-08-10, not of any catalog-wide job. Fix: a refresher that reads ' +
      'SKUs from parts.js (as bestbuy-dead-sku-audit.mjs does) and writes priceConfirmedAt. ' +
      'BESTBUY_API_KEY is already in GitHub secrets.',
  },

  msi: {
    unscheduled:
      'NO RUNNABLE PATH. ingest-msi-impact-v2.cjs is the only script maintaining this retailer and ' +
      'it hard-exits without IMPACT_ACCOUNT_SID / IMPACT_AUTH_TOKEN, neither of which is in GitHub ' +
      'secrets. No workflow invokes it. Not one of its 157 rows carries a stamp of any kind — the ' +
      'only retailer in the catalog with none. Decide: add the two Impact secrets and schedule the ' +
      'ingest, or drop deals.msi. Publishing never-confirmed prices is the option to rule out.',
  },
