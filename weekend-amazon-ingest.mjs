...
// Env-overridable so the halt paths can be positively tested (a check that has never
// tripped is unproven). Defaults are the spec's thresholds.
const HS_CEILING_FAIL = parseFloat(process.env.HS_CEILING_FAIL) || 0.30;   // >30% ceiling fail => stale ceiling => halt
const HS_ACCEPT_RATE = parseFloat(process.env.HS_ACCEPT_RATE) || 0.80;     // >80% accept => a gate isn't firing => halt
const HS_GROWTH_TOTAL = parseInt(process.env.HS_GROWTH_TOTAL) || 3000;     // cumulative additions cap
const HS_DETECTOR = parseFloat(process.env.HS_DETECTOR) || 0.05;           // >5% of new rows flagged by wrong-ASIN detector
