            ...
            {isExp&&<div style={{background:"var(--bg3)",borderRadius:"0 0 10px 10px",padding:"22px 24px",marginBottom:6,border:"1px solid var(--bdr)",borderTop:"none"}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1.4fr",gap:28}}>
                ...
                (p.c==="CPU"||p.c==="GPU"||p.c==="Motherboard")&&<div style={{background:"var(--bg4)",borderRadius:10,padding:"16px 18px"}}>
                  ...
                  {p.msrp&&p.msrp>$(p)&&<div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:6,background:"var(--bg4)",border:"1px solid var(--bdr)",marginTop:8}}><span style={{fontSize:17}}>💰</span><div style={{flex:1}}><div style={{fontFamily:"var(--ff)",fontSize:13,fontWeight:600,color:"var(--txt)"}}>Below MSRP</div><div style={{fontFamily:"var(--ff)",fontSize:13,color:"var(--dim)"}}>Was <span style={{textDecoration:"line-through"}}>${fmtPrice(p.msrp)}</span> → ${fmtPrice($(p))}</div></div><div style={{flex:1,display:"flex",justifyContent:"center"}}>{rr.length>1&&<span style={{fontFamily:"var(--ff)",fontSize:13,color:"var(--dim)",textAlign:"center",whiteSpace:"nowrap"}}>Save <span style={{color:"var(--mint)",fontWeight:600}}>${(rr[rr.length-1].price-rr[0].price).toFixed(2)}</span> at {rr[0].name} vs {rr[rr.length-1].name}</span>}</div><span style={{fontFamily:"var(--ff)",fontSize:17,fontWeight:700,color:"var(--mint)"}}>{Math.round((1-$(p)/p.msrp)*100)}% off</span><div style={{flexShrink:0,display:"flex",alignItems:"center",paddingLeft:4}}>{(p.r||p.reviews)&&<ReviewStars rating={p.r} reviews={p.reviews} size="md"/>}</div></div>}
                  {/* Product Image */}
