...
// Disclosure for EVERY retailer whose price on this product is a 3P marketplace listing.
...
// RETAILER-AWARE as of 2026-08-10: the Newegg case ingest writes marketplace rows with
// the same priceSource:'3p' contract, so the same component discloses them — wording and
// tooltip take the retailer name instead of hardcoding Amazon. One component, one contract:
// a new retailer only has to pass `retailer` and set priceSource.
function ThirdPartyBadge({deal,retailer="Amazon"}){
  ...
  return <span data-price-source="3p" data-price-retailer={retailer.toLowerCase()}
          ...
          <div onClick={()=>setExpanded(isExp?null:p.id)} style={{display:"flex",gap:12,padding:12,cursor:"pointer",minWidth:0}}>
            ...
            <div style={{flex:1,minWidth:0,display:"flex",flexDirection:"column",gap:4}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                ...
                {/* no 3P badge on mobile browse rows: this component is browse-only.
                    The mobile PRODUCT page routes through SearchPage, which keeps it. */}
              </div>
            ...
            {list.map(p=>{
              ...
              return <div key={p.id} onClick={()=>setExpanded(p.id)} style={{background:"var(--bg2)",border:"1px solid var(--bdr)",borderRadius:10,padding:12,cursor:"pointer",transition:"transform .15s, border-color .15s",display:"flex",flexDirection:"column",gap:8}} onMouseEnter={e=>{e.currentTarget.style.borderColor="var(--accent)";e.currentTarget.style.transform="translateY(-2px)";}} onMouseLeave={e=>{e.currentTarget.style.borderColor="var(--bdr)";e.currentTarget.style.transform="translateY(0)";}}>
                ...
                <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                  ...
                  {p.condition==="refurbished"&&<Tag color="var(--sky)">REFURB</Tag>}
                  {p.condition==="open-box"&&<Tag color="var(--violet)">OPEN BOX</Tag>}
                  {/* 3P disclosure belongs next to the price and the Buy button, not
                      repeated down a scanning list. SearchPage renders BOTH the browse list
                      AND the product page (singleProductId -> a one-item, pre-expanded list),
                      so this is GATED rather than deleted: the product page keeps the badge,
                      browse rows lose it. Applies to Amazon and Newegg equally. The
                      per-retailer badge beside each price/Buy link is untouched, and
                      priceSource:'3p' in the data is unchanged. */}
                  {singleProductId&&<ThirdPartyBadges p={p}/>}
                </div>
