// =============================================================================
//  Pro Rig Scanner Lite  —  portable console build
...
//
//  Tiny single-file console exe: reads hardware via WMI, builds the same
//  handoff URL the WPF scanner produces, opens the browser, exits.
//
//  Hardware-detection logic (GetCPU/GetGPU/GetRAM/GetStorage/GetMotherboard,
//  ScoreGPU/GuessVRAM and the data models) is copied VERBATIM from
//  pchw-scanner/MainWindow.xaml.cs so detection accuracy is identical.
//  The only intentional differences from the WPF BuildURL:
//    - user-question fields (budget, add_storage_gb, add_storage_type,
//      cooler_type) are OMITTED — those are now asked on the web page.
//    - use_case is kept and defaults to "gaming".
// =============================================================================
...
namespace ProRigScanner
{
    static class Program
    {
        ...
        const string _useCase = "gaming";   // matches the WPF default
        ...
        static int Main(string[] args)
        {
            ...
            string url = BuildURL(specs, baseUrl);

            // --print: dump the URL (and a spec summary) instead of opening the
            // browser — used to verify byte-compatibility against the old scanner.
            if (args.Contains("--print"))
            {
                Console.WriteLine($"CPU : {specs.CPU.Name} | {specs.CPU.Cores}c/{specs.CPU.Threads}t | {specs.CPU.MaxClockGHz:F1}GHz | {specs.CPU.Socket}");
                Console.WriteLine($"GPU : {specs.GPU.Name} | {specs.GPU.VRAM_MB / 1024}GB");
                Console.WriteLine($"RAM : {specs.RAM.TotalGB}GB {specs.RAM.Type} @ {specs.RAM.SpeedMHz}MHz | {specs.RAM.Sticks} sticks | {specs.RAM.UsedSlots}/{specs.RAM.TotalSlots} slots | {specs.RAM.SlotLabels}");
                Console.WriteLine($"MOBO: {specs.Motherboard.Manufacturer} {specs.Motherboard.Product}");
                foreach (var d in specs.Storage.Take(4))
                    Console.WriteLine($"DISK: {d.Model} | {d.SizeGB}GB | {d.Type}");
                Console.WriteLine();
        ...
        // ============================================================
        // URL BUILDER
        // Same JSON key names, same base64+urlencode, same URL shape as the
        // WPF scanner's BuildURL — EXCEPT the user-question fields (budget,
        // add_storage_gb, add_storage_type, cooler_type) are omitted; those
        // are now collected on the #upgrade web page.
        // ============================================================
        static string BuildURL(SystemSpecs specs, string baseUrl)
        {
        ...
        static string Clean(string s) => s.Trim().Replace("  ", " ").Replace("(R)", "").Replace("(TM)", "").Replace("(tm)", "");
        static string Truncate(string s, int max) => s.Length > max ? s[..(max - 2)] + ".." : s;
        static string Escape(string s) => s.Replace("\\", "\\\\").Replace("\"", "\\\"").Replace("\n", "").Replace("\r", "");

        // ============================================================
        // HARDWARE DETECTION — copied verbatim from MainWindow.xaml.cs
        // ============================================================
        ...
        static GPUInfo MockGPUDetection(string scenario)
        {
        ...
        static int ScoreGPU(string name, long vramMB, string pnp)
        {
            var n = name.ToUpper();
        ...
        static List<StorageInfo> GetStorage()
        {
        ...
        static MotherboardInfo GetMotherboard()
        {
        ...
        static long GuessVRAM(string n)
        {
    ...
    // ============================================================
    // DATA MODELS — copied verbatim from MainWindow.xaml.cs
    // ============================================================
